...
class Trainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.train_metrics = MetricTracker(
            "MPD_loss_discriminator",
            "MSD_loss_discriminator",
            "Discriminator_loss",
            "MPD_loss_generator",
            "MSD_loss_generator",
            "MEL_spectrogram_loss_generator",
            "MPD_features_loss_generator",
            "MSD_features_loss_generator",
            "Generator_loss",
        ...
        for batch_idx, batch in enumerate(
            ...
            if batch_idx % self.log_step == 0:
                ...
                self.writer.add_scalar(
                    "LR of discriminator", self.lr_scheduler_d.get_last_lr()[0]
                )
                self.writer.add_scalar(
                    "LR of generator", self.lr_scheduler_g.get_last_lr()[0]
                )
    ...
    def process_batch(self, batch, batch_idx, metrics: MetricTracker):
        ...
        batch["MPD_loss_discriminator"] = mpd_d_loss
        batch["MSD_loss_discriminator"] = msd_d_loss
        batch["Discriminator_loss"] = d_loss
        
        d_params = itertools.chain(self.model.mpd.parameters(), self.model.msd.parameters())
        batch["grad_norm_discriminator"] = torch.tensor([self.get_grad_norm(d_params)])
        ...
        batch["MPD_loss_generator"] = mpd_g_loss
        batch["MSD_loss_generator"] = msd_g_loss
        batch["MEL_spectrogram_loss_generator"] = mel_spec_g_loss
        batch["MPD_features_loss_generator"] = mpd_features_g_loss
        batch["MSD_features_loss_generator"] = msd_features_g_loss
        batch["Generator_loss"] = g_loss
        g_params = self.model.generator.parameters()
        batch["grad_norm_generator"] = torch.tensor([self.get_grad_norm(g_params)])
    ...
    def _log_audio(self, audio, sr, name):
        self.writer.add_audio(f"Sample_{name}", audio, sample_rate=sr)
