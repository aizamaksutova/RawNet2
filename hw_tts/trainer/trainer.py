...
import hw_tts.waveglow.utils as waveglow_utils

from hw_tts.waveglow import get_wav, get_waveglow
from hw_tts.synth.synth import synth
from hw_tts.synth.synth import get_text_data
...
from hw_tts.audio import params
...
class Trainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.test_data = get_text_data()

        self.loss = FastSpeechLoss()

        self.waveglow = waveglow_utils.get_WaveGlow().to(self.device)
    ...
    def _evaluation_epoch(self, epoch):
        ...
        for i, phn in tqdm(enumerate(self.test_data)):
            wav = synth(self.model, self.device, self.waveglow, phn)
    ...
    def _log_audio(self, name: str, wav):
        self.writer.add_audio(name, wav, sample_rate=params.sampling_rate)
